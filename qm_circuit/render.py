"""Matplotlib renderer: one line per element, schematic x axis (program order, not time).
...
block labels, marker labels) get vertical room reserved just above the region's top line, so they
never cross other lines, and each node only claims horizontal room on the lines it spans.
...
from .nodes import Align, Block, If, Loop, Op, Play, Program, Wait, children, elements
...
def wrap(parts: list[str], width: int) -> list[str]:
    ...
    for i, p in enumerate(parts):
        ...
        if texts and len(texts[-1]) + 1 + len(p) <= width:
            texts[-1] += " " + p
        else:
            texts.append(p)
    return texts
...
def elide(texts: list[str], n: int) -> list[str]:
    """At most ``n`` lines; the last kept one ends with ``...`` when some were dropped."""
    return texts if len(texts) <= n else texts[: n - 1] + [texts[n - 1].rstrip(",") + ", ..."]
...
def all_lines(n) -> bool:
    """``align()`` / ``wait(t)`` without elements act on every element of the program: so does any node holding one."""
    return isinstance(n, (Align, Wait)) and not n.targets or any(all_lines(c) for c in children(n))


def runs(indices: list[int]) -> list[list[int]]:
    """Split sorted line indices into runs of adjacent lines."""
    out: list[list[int]] = []
    for i in indices:
...
class _Layout:
    def __init__(self, ax, lines: list[str], body: list, expand: bool, s: Style, top: float = 0.0):
        """Lay out ``body`` with its highest drawing at y = ``top``."""
        self.ax, self.lines, self.expand, self.s = ax, lines, expand, s
        self.cur = {ln: 0.0 for ln in lines}
        self.xmax = 0.0
        self.above = {ln: 0.0 for ln in lines}  # room reserved above / below each line's band
        self.below = {ln: 0.0 for ln in lines}
        self.reserve(body, lines)
        self.y, y = {}, 0.0
        for i, ln in enumerate(lines):
            if i:
                clear = max(s.em(s.line_clear), self.below[lines[i - 1]] + self.above[ln] + s.em(s.min_clear))
                y -= s.em(s.box_h) + clear
            self.y[ln] = y
        shift = top - self.top(lines) - self.above[lines[0]]
        self.y = {ln: y + shift for ln, y in self.y.items()}
        self.ymax, self.ymin = top, self.bottom(lines) - self.below[lines[-1]]

    # ------------------------------------------------------------ geometry

    def span(self, n, ctx: list[str]) -> list[str]:
        """Contiguous lines from the first to the last touched one (``ctx`` if the node touches none)."""
        if all_lines(n):
            return self.lines
        idx = sorted(self.lines.index(e) for e in elements(n))
        return self.lines[idx[0] : idx[-1] + 1] if idx else ctx

    def top(self, lines) -> float:
        return max(self.y[ln] for ln in lines) + self.s.em(self.s.box_h) / 2

    def bottom(self, lines) -> float:
        return min(self.y[ln] for ln in lines) - self.s.em(self.s.box_h) / 2

    def stack(self, n, ctx, up: bool) -> float:
        """Height drawn beyond the band of the node's top (``up``) or bottom line."""
        s = self.s
    ...
    def x_at(self, lines) -> float:
        return max(self.cur[ln] for ln in lines) + self.s.em(self.s.gap)

    def advance(self, lines, x: float) -> None:
        for ln in lines:
            self.cur[ln] = max(self.cur[ln], x)
        self.xmax = max(self.xmax, x)

    def between(self, touched) -> list[str]:
        """Untouched lines inside the span of ``touched`` (crossed by connector lines)."""
        idx = [self.lines.index(t) for t in touched]
        return [ln for ln in self.lines[min(idx) : max(idx) + 1] if ln not in touched]
    ...
    def icon_w(self, icon: str | None, texts: list, h: float) -> float:
        """Box width for an icon, a separator and text ``texts`` of ``(text, font, family, weight, color)``."""
        s = self.s
        text = max((self.tw(t, f, fam, wt) for t, f, fam, wt, _ in texts), default=0.0)
        if icon is None:
            return text + 2 * s.em(s.pad)
        return h * s.icon + 2 * s.em(s.pad) + (text + 2 * s.em(s.pad) if texts else 0.0)

    def icon_content(self, x, y, w, h, icon: str | None, texts: list, dys: tuple, ink, gid) -> None:
        ...
        cx = (x_text + x + w) / 2
        offsets = [0.0] if len(texts) == 1 else [s.em(dys[0]), -s.em(dys[1])]
        for (text, font, family, weight, color), dy in zip(texts, offsets):
    ...
    def play(self, n: Play, gid) -> None:
        ...
        texts = [(n.pulse, s.font_pulse, s.sans, s.bold, c["pulse_text"])]
        if params:
            texts.append((params, s.font_param, s.mono, s.regular, c["param_text"]))
        icon = "measure" if n.measure else "play"
        x, y, h = self.x_at([n.element]), self.y[n.element], s.em(s.box_h)
        w = max(self.icon_w(icon, texts, h), self.icon_w(icon, [], h) + s.em(s.min_text_w) + 2 * s.em(s.pad))
        fill = c["measure" if n.measure else "pulse"]
        self.rbox(x, y - h / 2, w, h, gid, facecolor=fill, edgecolor=c["pulse_edge"], zorder=s.z_box)
        self.icon_content(x, y, w, h, icon, texts, (s.pulse_title_dy, s.pulse_param_dy), c["pulse_text"], gid)
        self.advance([n.element], x + w)

    def small_boxes(self, targets, name, args, edge, fill, dash, gid, color=None) -> None:
        """Same-width boxes at one x on each target line (wait, update_frequency, ...).
        ...
        texts = [sub] if args else []
        if icon is None:
            texts.insert(0, (name, s.font_small, s.sans, s.bold, edge))
        x, h = self.x_at(targets), s.em(s.small_box_h)
        w = self.icon_w(icon, texts, h)
        for t in targets:
            y = self.y[t]
            self.rbox(x, y - h / 2, w, h, gid, facecolor=fill, edgecolor=edge, linewidth=s.pt(s.lw_box),
                      linestyle=dash, zorder=s.z_box)
            self.icon_content(x, y, w, h, icon, texts, (s.small_title_dy, s.small_sub_dy), edge, gid)
    ...
    def marker(self, n: Op, ctx, gid) -> None:
        ...
        x = self.x_at(ctx) + w / 2
        self.vline(x, self.bottom(ctx), self.top(ctx), gid, s.lw_marker, color=s.colors["op_edge"], zorder=s.z_barrier)
    ...
    def align(self, n: Align, ctx, gid) -> None:
        ...
        segs = runs(sorted(self.lines.index(t) for t in touched))
        style = dict(color=s.colors["align"], zorder=s.z_barrier)
        for i, seg in enumerate(segs):
            lines = [self.lines[j] for j in seg]
            y0, y1 = self.bottom(lines) - s.em(s.overhang), self.top(lines) + s.em(s.overhang)
            self.vline(x, y0, y1, gid, s.lw_align, linestyle=s.dash_align, **style)
            if i:
                prev = [self.lines[segs[i - 1][-1]]]
    ...
    def block(self, n: Block, ctx, gid) -> None:
        """Collapsed helper: one box per run of adjacent touched lines, joined by a thin connector."""
        s = self.s
        touched = [ln for ln in self.lines if ln in elements(n)] or ctx
        if all_lines(n):  # the helper synchronises every line: one box over its whole span
            touched = self.span(n, ctx)
        segs = [[self.lines[j] for j in r] for r in runs([self.lines.index(t) for t in touched])]
        main = max(segs, key=len)
        edge = s.em(s.edge)
        fit = max(1, int((self.top(main) - self.bottom(main) + 2 * edge - s.em(s.block_text_margin)) / s.em(s.line)))
        width = s.block_chars
        texts = wrap(n.params, width)
        while len(texts) > fit and width < s.block_chars_max:
            width += s.block_chars_step
            texts = wrap(n.params, width)
        texts = elide(texts, fit)
        w = max(self.tw(n.func, s.font_block, s.sans, s.bold), *(self.tw(ln, s.font_param, s.mono) for ln in texts),
                0) + 2 * s.em(s.pad)
        crossed = self.between(touched)
        x = self.x_at(touched)
        if crossed:  # the connector at the box center must clear the crossed lines
        ...
        for i, seg in enumerate(segs):
            ...
            if i:
                self.vline(x + w / 2, y1, self.bottom(segs[i - 1]) - edge, gid, s.lw_connector,
                           color=s.colors["block_edge"], zorder=s.z_barrier)
            text = texts if seg is main else []
    ...
    def repeat(self, x: float, lines, dots, y0: float, y1: float, start: bool, gid) -> float:
        ...
        ys = [self.y[ln] + d for ln in lines if ln in dots for d in (e(s.repeat_dot_dy), -e(s.repeat_dot_dy))]
    ...
    def region(self, n, span, hi, lo, level: int, gid) -> None:
        ...
        texts = label_lines(n, s.label_chars, s.label_lines)
        row_y = [hi - (i + 1) * e(s.row) + e(s.label_dy) for i in range(len(texts))]
        x0 = self.x_at(span)
        label_w = max(self.tw(ln, s.font_small, s.mono) for ln in texts)
        if isinstance(n, Loop):
            dots = span if all_lines(n) else elements(n) or span
...
def render(program: Program, expand: bool = False, title: str | None = None, style: Style | dict | None = None,
    ...
    ``max_width`` (inches): wrap top-level statements into several rows of lines (each row restarts at x=0).
    """
    s = make_style(style)
    e = s.em
    lines = program.lines or ["(no elements)"]
    fig = Figure(dpi=s.dpi)
    fig.patch.set_facecolor(s.colors["background"])
    ax = fig.add_axes((0, 0, 1, 1))
    ax.axis("off")
    lays, top = [], 0.0
    for body in rows(program.body, lines, expand, s, max_width):
        lays.append(_Layout(ax, lines, body, expand, s, top))
        lays[-1].place(body, lines)
        top = lays[-1].ymin - e(s.row_gap)

    title = program.name if title is None else title
    name_w = max(lays[0].tw(ln, s.font_line, s.mono, s.bold) for ln in lines)
    xmin, xmax = -name_w - e(s.line_name_gap) - e(s.margin_left), max(lay.xmax for lay in lays) + e(s.margin_right)
    ...
    for lay in lays:
        for ln in lines:
            y = lay.y[ln]
            ax.plot([-e(s.line_start), xmax - e(s.line_end)], [y, y], color=s.colors["line"],
                    linewidth=s.pt(s.lw_line), zorder=s.z_line, gid="line")
            ax.text(-e(s.line_name_gap), y, ln, ha="right", va="center", fontsize=s.pt(s.font_line),
                    fontweight=s.bold, family=s.mono, color=s.colors["text"], gid="line")
    if title:
